(function() {
    ...
    const chatHTML = `
        <div id="grok-container" style="position:fixed; bottom:20px; right:20px; z-index:999999; font-family: sans-serif;">
            ...
            <div id="grok-window" style="display:none; position:absolute; bottom:80px; right:0; width:330px; height:450px; background:white; border-radius:12px; box-shadow:0 8px 24px rgba(0,0,0,0.2); flex-direction:column; overflow:hidden; border: 1px solid #eee;">
                ...
                <input type="text" id="grok-hp" style="display:none !important" tabindex="-1">
    ...
    function addMessage(sender, text) {
        ...
        m.style.cssText = `padding:8px 12px; border-radius:8px; max-width:80%; font-size:14px; line-height:1.4; ${isUser ? 'align-self:flex-end; background:#dcf8c6;' : 'align-self:flex-start; background:white; border:1px solid #ddd;'}`;
    ...
    async function handleSend() {
        ...
        if (isWaiting || !text || document.getElementById('grok-hp').value !== "") return;
        ...
        try {
            const res = await fetch(CONFIG.scriptUrl, {
                method: 'POST',
                // Enviamos como text/plain para simplificar o CORS no Google Apps Script
